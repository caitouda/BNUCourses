...
static unsigned int block_buf[BLOCKSIZ];   //每个块的大小为BLOCHSIZ=512
/**********************************************************
函数：balloc
...
unsigned int balloc(){
	...
	//如果没有空闲盘块
	if (filsys.s_nfree == 0){  //空闲块数=s_nfree
		printf("\nDisk Full!!!\n");
		return DISKFULL;
	}
	free_block = filsys.s_free[filsys.s_pfree]; //取堆栈中的盘块号 
	//s_pfree=空闲块指针,指向当前堆栈空闲的位置   s_free=空闲块堆栈
	if (filsys.s_pfree == NICFREE-1){		    //如果堆栈只剩一个块
		...
		//s_free[NICFREE]  	/*空闲块堆栈*/     NICFREE=50
