...
public:
	...
	void Uniontwo(SeqList<T> &a,SeqList<T> &b);
	void Removethesame(SeqList &a);//移去相同的
private:
	T* data;  // 存放陣列
...
void SeqList<T>::Removestot(int s,int t)
{
	if(s<1||t>last+1)
	{
		cout<<"error:illegal s or t!!!"<<endl;
		return;
	}
	T mm;
	for(int i=t;i>=s;i--)
...
template <class T>
void SeqList<T>::Uniontwo(SeqList<T> &a,SeqList<T> &b)
{
    int i=0;
    int j=0;
    int num=0;
    ...
    while(i<=last&&j<=lasta)
    {
        if(data[i]>a.data[j])
        {
            b.Insert(num++,a.data[j]);
            j++;
        }
        else
        {
...
template <class T>
void SeqList<T>::Removethesame(SeqList<T> &a)
{
	...
	i=0;
	j=0;
	while(i<=a.last&&j<=a.last)
	{
		while(a.data[i]==a.data[j]&&j<=a.last)
		{
			j++;
		}
		last++;
